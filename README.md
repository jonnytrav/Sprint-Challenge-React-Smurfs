...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
      Server side routing makes a request and returns a new doc that requires a page refresh.. it only returns exactly what you need. Client side is when the data the client needs is loaded into the code beforehand and requests are handled internally and do not require a refresh.
...
* [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
      Create would use the post method, read would use get, update would use put, and delete uses delete.
